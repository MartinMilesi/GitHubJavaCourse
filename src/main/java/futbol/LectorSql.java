...
public class LectorSql {
	...
	public void obtenerDatos(String rutaArchivoConfiguracion){
		...
		String consultaResultados = "select * from futbol.resultados";
		String consultaPronosticos = "select * from futbol.pronosticos";
		...
		ArrayList<ArchivoPronosticos> pronosticos =  new ArrayList<ArchivoPronosticos>();
		 ...
		 ConectorSql.setDB_URL(config.getProperty("DB_URL"));
		 ConectorSql.setUSER(config.getProperty("USER"));
		 ConectorSql.setPASS(config.getProperty("PASS"));
		
		try {
			conexion = DriverManager.getConnection(ConectorSql.getDB_URL(),ConectorSql.getUSER(),ConectorSql.getPASS());
			consulta = conexion.createStatement();
			
			ResultSet resultado = consulta.executeQuery(consultaResultados);
			while(resultado.next()) {
				ArchivoResultados archivoResultados = new ArchivoResultados();
				archivoResultados.setIdResultados(Integer.parseInt(resultado.getString("idresultados")));
				archivoResultados.setFase(Integer.parseInt(resultado.getString("fase")));
				archivoResultados.setRonda(Integer.parseInt(resultado.getString("ronda")));
				archivoResultados.setEquipo1(resultado.getString("equipo1"));
				archivoResultados.setEquipo2(resultado.getString("equipo2"));
				archivoResultados.setCantGoles1(Integer.parseInt(resultado.getString("cantgoles1")));
				archivoResultados.setCantGoles2(Integer.parseInt(resultado.getString("cantgoles2")));
				resultados.add(archivoResultados);
			}
			
			ResultSet pronostico = consulta.executeQuery(consultaPronosticos);
			while(pronostico.next()) {
				ArchivoPronosticos archivoPronosticos = new ArchivoPronosticos();
				archivoPronosticos.setIdPronostico(pronostico.getString("idpronosticos"));
				archivoPronosticos.setParticipante(pronostico.getString("participante"));
				archivoPronosticos.setEquipo1(pronostico.getString("equipo1"));
				archivoPronosticos.setEquipo2(pronostico.getString("equipo2"));
				archivoPronosticos.setGana1(pronostico.getString("gana1"));
				archivoPronosticos.setGana2(pronostico.getString("gana2"));
				archivoPronosticos.setEmpata(pronostico.getString("empata"));
